...
圆锥曲线是以不同角度切割圆锥所得到的曲线族：圆、椭圆、抛物线和双曲线。你已经知道圆（$x^2 + y^2 = r^2$），也见过抛物线（$y = ax^2$）。本课探索这个家族的另外两个成员——**椭圆**和**双曲线**——并揭示它们如何通过*焦点*性质相互联系。可以把椭圆想象成被拉伸的圆，双曲线想象成它们的"内外翻转"版本。这两种曲线都通过涉及两个特殊点（称为**焦点**）的距离条件来定义。
